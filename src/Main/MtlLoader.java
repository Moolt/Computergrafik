...
public class MtlLoader {
	...
	public float[] getKa(String namepass) {
		System.out.println("MtlLoader Materials.size = "+Materials.size());
			...
			while (((newline = br.readLine()) != null)) {
				...
				if (newline.length() > 0) {
					...
					if (newline.charAt(0) == 'K' && newline.charAt(1) == 'a') {
						System.out.println("MtlLoader in Ka ");
						float[] coords = new float[3];
						String[] coordstext = new String[4];
						coordstext = newline.split("\\s+");
						System.out.println("MtlLoader coordstext.length = "+coordstext.length);
						for (int i = 1;i < coordstext.length;i++) {
							coords[i-1] = Float.valueOf(coordstext[i]).floatValue();
							System.out.println("MtlLoader coords "+i+"  "+coords[i-1]);
						}
						matset.Ka = coords;
					}
